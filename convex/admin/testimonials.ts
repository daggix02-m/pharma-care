...
export const getTestimonials = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.status !== undefined) {
      ...
      testimonials = await ctx.db.query("testimonials").take(args.limit || 50);
    }
    ...
    const stats = {
      ...
      avgRating:
        allTestimonials.length > 0
          ? allTestimonials.reduce((sum, t) => sum + t.starRating, 0) /
...
export const getTestimonialById = query({
  ...
  handler: async (ctx, args) => {
    ...
    const reviewer = testimonial.reviewedBy
      ? await ctx.db.get(testimonial.reviewedBy)
      : null;
