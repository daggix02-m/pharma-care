...
export function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSuccess(
        'Password reset is handled by Clerk. Please use the "Forgot Password" link on the login page.'
      );
      setEmail('');
    } catch (err: unknown) {
      const errorMessage =
      ...
      <div className='space-y-6'>
        ...
        {success && (
          <div className='p-4 bg-accent/10 border border-accent/20 rounded-xl flex items-start gap-3'>
            ...
            <div>
              <p className='text-sm font-medium text-accent-foreground'>Success</p>
              <p className='text-sm text-accent-foreground/80 mt-1'>{success}</p>
            </div>
          </div>
        )}

        {verificationCode && isDevelopmentMode && (
