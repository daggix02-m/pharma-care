...
export function TestimonialSubmission() {
  ...
  const handleSubmit = async () => {
    ...
    if (content.length < (guidelines?.minLength || 50)) {
      toast.error(`Testimonial must be at least ${guidelines?.minLength || 50} characters`);
      return;
    }
    ...
    try {
      ...
      toast.success('Testimonial submitted successfully! It will be reviewed and published soon.');
      setContent('');
  ...
  if (submissionStatus === undefined || guidelines === undefined || myTestimonials === undefined) {
    return (
    ...
    <div className='space-y-6'>
      <div>
        ...
        <p className='text-sm text-muted-foreground mt-1'>
          Help other pharmacy owners learn about PharmaCare by sharing your testimonial
        </p>
      </div>
      ...
      <div className='grid gap-6 lg:grid-cols-2'>
          ...
          <CardContent className='space-y-6'>
            {!canSubmit ? (
              <div className='text-center py-8'>
                ...
                <p className='text-muted-foreground'>{submissionStatus?.reason}</p>
                {submissionStatus?.existingTestimonial && (
                  <div className='mt-4 p-4 bg-muted rounded-lg text-left'>
                    <p className='text-sm font-medium'>Your existing testimonial:</p>
                    <p className='text-sm text-muted-foreground mt-1'>
                      Status:{' '}
                      <Badge
                        variant={
                          submissionStatus.existingTestimonial.status === 'approved'
                            ? 'default'
                            : submissionStatus.existingTestimonial.status === 'pending'
                              ? 'secondary'
                              : 'destructive'
                        }
                      >
              ...
              <>
                ...
                <div className='space-y-2'>
                  ...
                  />
                  <Progress value={(content.length / guidelines.maxLength) * 100} className='h-1' />
                  {content.length < guidelines.minLength && content.length > 0 && (
                ...
                <Button
                  ...
                  disabled={isSubmitting || content.length < guidelines.minLength}
                  className='w-full'
                  size='lg'
                >
                  {isSubmitting ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Track the status of your testimonials</CardDescription>
          </CardHeader>
          <CardContent>
            {myTestimonials.length === 0 ? (
              ...
              <div className='space-y-4'>
                {myTestimonials.map((testimonial) => (
                  <div key={testimonial._id} className='p-4 border rounded-lg space-y-3'>
                    ...
                    <p className='text-xs text-muted-foreground'>
                      Submitted on {new Date(testimonial.submittedAt).toLocaleDateString()}
                    </p>
                    {testimonial.adminNotes && (
                      <div className='text-xs text-muted-foreground bg-muted p-2 rounded'>
                        <span className='font-medium'>Admin Note:</span> {testimonial.adminNotes}
                      </div>
                    )}
                  </div>
                ))}
