import { CreditCard, Calendar, CheckCircle, Download, FileText, DollarSign } from 'lucide-react';
...
export function SubscriptionBillingSection({ pharmacy }: SubscriptionBillingSectionProps) {
  const currentPlan =
    planDetails[pharmacy.subscriptionTier as keyof typeof planDetails] || planDetails.basic;

  const paymentHistory = [
    {
      id: '1',
      date: '2026-02-22',
      amount: pharmacy.monthlyCost || 0,
      status: 'paid',
          ...
          <Card className='border-2 border-border'>
            ...
            <CardContent className='space-y-4'>
              <div className='space-y-3'>
                ...
                <BillingItem label='Monthly Cost' value={`$${pharmacy.monthlyCost || 0}`} />
              ...
              <div className='p-4 bg-muted/30 rounded-lg'>
                <div className='flex items-center justify-between'>
                  <div>
                    ...
                    <Badge
                      variant={pharmacy.paymentStatus === 'paid' ? 'default' : 'destructive'}
                      className='mt-1'
                    >
                      {pharmacy.paymentStatus?.toUpperCase() || 'N/A'}
                    </Badge>
                  </div>
                  {pharmacy.paymentStatus !== 'paid' && <Button size='sm'>Pay Now</Button>}
                </div>
              </div>
            </CardContent>
          </Card>
...
function PlanCard({ plan, isCurrent }: PlanCardProps) {
  return (
    <Card
      className={cn(
        'border-2',
        isCurrent ? 'border-[hsl(var(--medical-teal))] shadow-lg' : 'border-border'
      )}
    >
      <CardHeader
        className={cn(
      ...
      >
        <div className='flex items-center justify-between'>
          <CardTitle className={cn('text-lg font-semibold', isCurrent && 'text-white')}>
            {plan.name} Plan
          </CardTitle>
          {isCurrent && <Badge className='bg-white text-[hsl(var(--medical-teal))]'>Current</Badge>}
        </div>
        <div className='mt-4'>
          <span className={cn('text-3xl font-bold', isCurrent && 'text-white')}>${plan.price}</span>
          <span className={cn('text-sm text-muted-foreground ml-1', isCurrent && 'text-white/80')}>
            /month
          </span>
        </div>
      </CardHeader>
