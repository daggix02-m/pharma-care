...
export function Transactions() {
  ...
  const handleViewDetails = (transactionId: Id<'sales'>) => {
    const found = transactions.find((t: Transaction) => t._id === transactionId);
  ...
  const getTransactionTypeBadge = (status: string) => {
    ...
    const typeInfo = types[status] || { label: 'Unknown', color: 'bg-gray-100 text-gray-800' };
  ...
  return (
    <div className='space-y-6 p-4 md:p-8'>
                ...
                <tbody>
                  {filteredTransactions.map((transaction: Transaction) => (
                    <tr key={transaction._id} className='border-b hover:bg-muted/50'>
                      ...
                      <td className='py-3 px-4'>{getTransactionTypeBadge(transaction.status)}</td>
                      <td className='py-3 px-4'>#{transaction._id || 'N/A'}</td>
                      ...
                      <td className='py-3 px-4 text-sm'>{transaction.customerName || 'Walk-in'}</td>
      ...
      {selectedTransaction && (
        <Card className='mt-6'>
          <CardHeader>
            <CardTitle>Transaction Details - #{selectedTransaction._id}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className='space-y-4'>
              <div className='grid grid-cols-2 gap-4'>
                ...
                <div>
                  ...
                  <p className='font-medium'>#{selectedTransaction._id || 'N/A'}</p>
                </div>
                <div>
                  <p className='text-sm text-muted-foreground'>Date</p>
                  <p className='font-medium'>
                    {selectedTransaction._creationTime
                      ? new Date(selectedTransaction._creationTime).toLocaleString()
                      : 'N/A'}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className='font-medium'>ETB {selectedTransaction.totalAmount?.toFixed(2)}</p>
                </div>
                <div>
