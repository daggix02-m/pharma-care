  ...
  return (
    <div className="space-y-6 p-4 md:p-8 animate-fade-up">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Start and end your daily cashier shift</p>
      </div>
      ...
      <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
        ...
        <Card className="rounded-[2.5rem] border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <CardHeader className={`${activeSession ? 'bg-emerald-600' : 'bg-indigo-600'} text-white p-8`}>
                <div className="flex items-center justify-between">
                    <CardTitle className="text-2xl font-bold flex items-center gap-3">
                        {activeSession ? <CheckCircle2 className="w-8 h-8" /> : <Play className="w-8 h-8" />}
                ...
                {activeSession ? (
                    <form onSubmit={handleEndSession} className="space-y-6">
                        <div className="grid grid-cols-2 gap-8 mb-8">
                            <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                                ...
                                <p className="text-2xl font-black text-slate-900">ETB {activeSession.opening_balance?.toFixed(2)}</p>
                            </div>
                            <div className="p-6 bg-indigo-50 rounded-3xl border border-indigo-100">
                                <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">Current Sales</p>
                                <p className="text-2xl font-black text-indigo-600">ETB {(activeSession.total_sales || 0).toFixed(2)}</p>
                            </div>
                        </div>

                        <div className="space-y-4">
                            <div className="space-y-2">
                                <Label className="text-xs font-bold uppercase tracking-wider text-slate-500 ml-1">Expected Cash in Drawer</Label>
                        ...
                        <Button 
                            ...
                            className="w-full h-14 rounded-2xl bg-red-600 hover:bg-red-700 text-white font-bold uppercase tracking-widest shadow-xl shadow-red-500/20"
                        >
                            {endingSession ? <Loader2 className="w-5 h-5 animate-spin mr-2" /> : <Square className="w-5 h-5 mr-2" />}
                            End Shift & Reconcile
                        </Button>
                    </form>
                ) : (
                    <form onSubmit={handleStartSession} className="space-y-8">
                        <div className="p-8 bg-indigo-50 rounded-[2rem] border border-indigo-100 flex items-center gap-6">
                            ...
                            <div>
                                <h4 className="font-bold text-slate-900 text-lg">Opening Cash</h4>
                                <p className="text-sm text-slate-500">Count the starting cash in your drawer</p>
                            </div>
                        </div>

                        <div className="space-y-3">
                            <Label className="text-xs font-bold uppercase tracking-wider text-slate-500 ml-1">Opening Balance (ETB)</Label>
                            <Input 
                                type="number"
                        ...
                        <Button 
                            ...
                            className="w-full h-14 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold uppercase tracking-widest shadow-xl shadow-indigo-500/20"
                        >
                            {startingSession ? <Loader2 className="w-5 h-5 animate-spin mr-2" /> : <Play className="w-5 h-5 mr-2" />}
                            Start Shift
                        </Button>
                    </form>
                )}
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            <CardTitle className="text-lg flex items-center gap-2">
                                <History className="w-5 h-5 text-indigo-600" /> Shift History
                            </CardTitle>
                            <CardDescription>Your last few completed shifts</CardDescription>
                        </div>
                    </div>
                </CardHeader>
                <CardContent className="p-0">
                    <div className="divide-y divide-slate-50">
                        {Array.isArray(sessionData) && sessionData.filter(s => s.status !== 'active').slice(0, 5).map((s, i) => (
                            <div key={s.id || i} className="p-6 hover:bg-slate-50/50 transition-colors group">
                                <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="font-bold text-slate-900">{new Date(s.start_time || s.created_at).toLocaleDateString()}</p>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                                {new Date(s.start_time || s.created_at).toLocaleTimeString()} - {s.end_time ? new Date(s.end_time).toLocaleTimeString() : 'N/A'}
                                            </p>
                                        </div>
                                    </div>
                                    <Badge className="bg-slate-100 text-slate-600 border-0 uppercase text-[9px] font-black tracking-tighter">CLOSED</Badge>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="px-4 py-2 bg-slate-50 rounded-xl border border-slate-100/50">
                                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Sales</p>
                                        <p className="text-sm font-bold text-slate-700">ETB {s.total_sales?.toFixed(2)}</p>
                                    </div>
                                    <div className="px-4 py-2 bg-indigo-50 rounded-xl border border-indigo-100/50">
                                        <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-0.5">Balance</p>
                                        <p className="text-sm font-bold text-indigo-600">ETB {s.closing_balance?.toFixed(2)}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
                ...
                <div className="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-white/80 leading-relaxed">
                            Always ensure your cash drawer matches the recorded transactions. Reported discrepancies are audited daily by management.
                        </p>
                    </div>
                </div>
            </Card>
