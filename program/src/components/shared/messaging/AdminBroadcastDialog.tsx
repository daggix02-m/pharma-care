...
import { Megaphone, Users, Shield, CheckCircle2, X, Send, Loader2 } from 'lucide-react';
...
type TargetAudience = 'all_users' | 'all_owners' | 'specific_plan' | 'specific_pharmacy';
...
export function AdminBroadcastDialog({ open, onOpenChange }: AdminBroadcastDialogProps) {
  const [targetAudience, setTargetAudience] = React.useState<TargetAudience>('all_owners');
  const [specificValue, setSpecificValue] = React.useState('');
  const [messageType, setMessageType] = React.useState<MessageType>('announcement');
  ...
  const handleSend = async () => {
    ...
    try {
      await sendAdminBroadcast({
        ...
        targetAudience,
        priority:
          messageType === 'compliance_alert' || messageType === 'security_notice'
            ? 'high'
            : 'normal',
      });
  ...
  const canProceed = () => {
    if (step === 1) {
      return targetAudience !== 'specific_plan' && targetAudience !== 'specific_pharmacy';
    }
        ...
        <div className='relative h-[700px] flex flex-col'>
                ...
                {step === 1 && (
                  ...
                  >
                    <div className='space-y-4'>
                      ...
                      <RadioGroup value={targetAudience} onValueChange={handleTargetAudienceChange}>
                      ...
                      {targetAudience === 'specific_plan' && (
                        <div className='space-y-2'>
                          <Label htmlFor='plan-input'>Subscription Plan Code</Label>
                          <Input
                            id='plan-input'
                        ...
                        <div className='space-y-2'>
                          <div className='flex items-center gap-2'>
                            ...
                            <span className='text-xs text-slate-500'>
                              to: {TARGET_OPTIONS.find((t) => t.value === targetAudience)?.label}
                            </span>
                          </div>
                          <h4 className='font-semibold text-slate-900'>{title || '<Untitled>'}</h4>
          ...
          <DialogFooter className='border-t border-slate-200 bg-white/95 backdrop-blur-sm px-6 py-4'>
            <div className='flex gap-3'>
              <Button
                variant='outline'
                onClick={() => setStep(Math.max(1, (step - 1) as 1 | 2) as 1 | 2 | 3)}
              ...
              <Button
                ...
                disabled={
                  (step === 3 && (!title.trim() || !message.trim())) || !canProceed() || isSending
                }
                className={cn(
