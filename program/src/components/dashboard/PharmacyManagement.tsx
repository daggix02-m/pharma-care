...
interface PharmacyManagementProps {
  ...
  error?: any;
  onRetry?: () => void;
  onAddPharmacy?: (data: any) => void;
  onEditPharmacy?: (data: any) => void;
...
export function PharmacyManagement({
  ...
  const [expandedPharmacies, setExpandedPharmacies] = useState(new Set<string>());
  ...
  const handleEditPharmacy = () => {
    if (onEditPharmacy && selectedItem?.id !== undefined) {
      onEditPharmacy({ id: selectedItem.id, ...formData });
    } else {
  ...
  const handleDeletePharmacy = () => {
    if (onDeletePharmacy && selectedItem?.id !== undefined) {
  ...
  const openEditPharmacyDialog = (pharmacy: any) => {
  ...
  const openDeletePharmacyDialog = (pharmacy: any) => {
    setSelectedItem(pharmacy);
      ...
      <div className='w-full max-w-7xl mx-auto p-4'>
        <div className='flex flex-col items-center justify-center h-96 space-y-4'>
          ...
          <p className='text-muted-foreground text-center max-w-md'>
            There are no pharmacies configured in the system. Contact your administrator or add a
            pharmacy to get started.
          </p>
          {onRetry && (
            <Button onClick={onRetry} variant='outline'>
              <Edit className='h-4 w-4 mr-2' />
        ...
        <AnimatedWrapper>
          <div className='flex justify-between items-center'>
            <div>
              ...
              <p className='text-muted-foreground mt-1'>Manage pharmacies and staff</p>
            </div>
          ...
          <CardContent className='p-6'>
            <div className='space-y-4'>
              {pharmacies.map((pharmacy: any, index: number) => {
                ...
                const managers = pharmacy.staff?.filter((s: any) => s.role === 'Manager') || [];
                const employees = pharmacy.staff?.filter((s: any) => s.role === 'Employee') || [];
                            ...
                            {managers.length > 0 && (
                              <div>
                                ...
                                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                                  {managers.map((staff: any, index: number) => (
                              ...
                              <div>
                                ...
                                <div className='overflow-x-auto'>
                                  <Table>
                                    ...
                                    <TableBody>
                                      {employees.map((staff: any, index: number) => (
        ...
        <DialogContent className='sm:max-w-[425px]'>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              be undone. This action is not available for admin role.
            </DialogDescription>
          </DialogHeader>
...
function StaffCard({ staff }: StaffProps) {
  return (
