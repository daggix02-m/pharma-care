...
export const initializeLandingPageContent = mutation({
  handler: async (ctx) => {
    ...
    if (!existing) {
      ...
      for (const [sectionKey, content] of Object.entries(DEFAULT_LANDING_PAGE_CONTENT)) {
    ...
    // Initialize sections
    const existingSections = await ctx.db.query('landing_page_sections').collect();
...
export const updateHeroSection = mutation({
  ...
  handler: async (ctx, args) => {
    const user = await checkAdminAuth(ctx);
...
export const toggleSectionVisibility = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return { success: true, sectionId: args.sectionId, isEnabled: args.isEnabled };
  },
});
...
export const getLandingPageAnalytics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const analytics = await ctx.db
      ...
      .withIndex('by_timestamp', (q) => q.gte('timestamp', cutoffDate.getTime()))
      .collect();

    const sectionStats: Record<string, { views: number; clicks: number; interactions: number }> =
      {};
    ...
    for (const event of analytics) {
      ...
      if (event.eventType === 'interaction') sectionStats[sectionId].interactions++;
    }
