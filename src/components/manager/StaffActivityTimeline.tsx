...
const getActivityIcon = (action: string) => {
  ...
  if (lowerAction.includes('login') || lowerAction.includes('sign in'))
    return { icon: LogIn, color: 'text-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-500/10' };
  if (lowerAction.includes('logout') || lowerAction.includes('sign out'))
    return { icon: LogOut, color: 'text-slate-500', bg: 'bg-slate-50 dark:bg-slate-500/10' };
  ...
  if (lowerAction.includes('update') || lowerAction.includes('edit'))
    return { icon: FileEdit, color: 'text-amber-500', bg: 'bg-amber-50 dark:bg-amber-500/10' };
  if (lowerAction.includes('delete') || lowerAction.includes('remove'))
    return { icon: Trash2, color: 'text-red-500', bg: 'bg-red-50 dark:bg-red-500/10' };
  if (lowerAction.includes('sale') || lowerAction.includes('transaction'))
    return { icon: TrendingUp, color: 'text-indigo-500', bg: 'bg-indigo-50 dark:bg-indigo-500/10' };
      ...
      {activities.map((activity: any, index: number) => {
        ...
        return (
          <div key={activity.id || index} className='relative flex gap-6 items-start'>
            {/* Timeline point */}
            <div
              className={`relative z-10 w-10 h-10 rounded-2xl ${bg} flex items-center justify-center shrink-0 shadow-sm border border-white dark:border-slate-900`}
            >
...
const Server = ({ className }: ServerIconProps) => (
  <svg
