...
import { BroadcastStepTarget } from "./BroadcastStepTarget";
import { BroadcastStepMessageType } from "./BroadcastStepMessageType";
import { BroadcastStepCompose } from "./BroadcastStepCompose";
...
export function AdminBroadcastDialog({
  ...
  const canProceedStepOne = () => {
    if (targetAudience === "specific_plan") return selectedPlanCodes.length > 0;
  ...
  const canProceed = () => {
    if (step === 1) return canProceedStepOne();
