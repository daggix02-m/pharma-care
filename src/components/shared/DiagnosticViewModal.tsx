...
export function DiagnosticViewModal({
  ...
  const startSessionMutation = useMutation(
    (api.admin as any).startDiagnosticSession,
  );
  const endSessionMutation = useMutation(
    (api.admin as any).endDiagnosticSession,
  );
  ...
  const handleStartSession = async () => {
    if (!startReason.trim()) {
      toast.error(
        "Please provide a reason for starting the diagnostic session",
      );
      return;
    }
  ...
  const SessionViewContent = () => {
    ...
    const sessionData = sessionId
      ? session.find((s: any) => s._id === sessionId)
      : activeSession;
              ...
              <CardHeader>
                <CardTitle className="text-base font-semibold flex items-center gap-2">
                  ...
                  <Badge variant="destructive">
                    {sessionData.blockedActions.length}
                  </Badge>
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  {sessionData.blockedActions.map(
