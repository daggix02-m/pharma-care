...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn('fixed inset-0 z-[200] bg-black/80 transition-opacity duration-200', className)}
    {...props}
    ref={ref}
  />
));
...
const sheetVariants = cva('fixed z-[200] gap-4 bg-sidebar transition ease-in-out', {
  variants: {
    side: {
      ...
      right: 'inset-y-0 right-0 h-screen min-h-screen w-3/4 border-l sm:max-w-sm p-6',
    },
  },
  defaultVariants: {
    side: 'right',
...
>(({ side = 'right', className, children, ...props }, ref) => {
  const combinedRef = React.useRef<React.ElementRef<typeof SheetPrimitive.Content>>(null);
      ...
      mutations.forEach((mutation) => {
        if (mutation.type === 'attributes' && mutation.attributeName === 'data-state') {
          ...
          if (state === 'open' && target === combinedRef.current) {
            gsap.fromTo(
              combinedRef.current,
              { opacity: 0, x: side === 'right' ? 100 : side === 'left' ? -100 : 0 },
