...
interface Sale {
  ...
  customerName?: string;
  totalAmount?: number;
  items?: Array<{ medicineId: Id<'medicines'>; quantity: number; price: number }>;
}
                ...
                <tbody>
                  {filteredSales.map((sale) => (
                    <tr key={sale._id} className='border-b hover:bg-muted/50'>
                      <td className='py-3 px-4 font-medium'>#{sale._id}</td>
                      <td className='py-3 px-4 text-sm'>{sale.customerName || 'Walk-in'}</td>
                      ...
                      <td className='py-3 px-4 text-sm'>{sale.items?.length || 0} items</td>
                      <td className='py-3 px-4'>
                        <div className='flex items-center gap-2'>
                          <Package className='h-4 w-4 text-muted-foreground' />
                          <span className='font-medium'>ETB {sale.totalAmount?.toFixed(2)}</span>
                        </div>
                      </td>
                ...
                <div>
                  ...
                  <div className='grid grid-cols-2 gap-4'>
                    ...
                    <div>
                      ...
                      <p className='font-medium'>ETB {selectedSale.totalAmount?.toFixed(2)}</p>
                    </div>
                  </div>
                </div>
                        ...
                        >
                          <div className='flex-1'>
                            ...
                            <p className='text-sm text-muted-foreground'>
                              Available: {item.quantity} | Price: ETB {item.price?.toFixed(2)}
                            </p>
                          </div>
                          <div className='flex items-center gap-3'>
                            <input
                              ...
                              onChange={() => handleItemSelect(item.medicineId.toString())}
                              className='h-4 w-4'
                            />
                            <input
                              type='number'
                              min='1'
                              max={item.quantity || 1}
                              value={selectedItems[item.medicineId.toString()]?.quantity || 1}
                              onChange={(e) =>
