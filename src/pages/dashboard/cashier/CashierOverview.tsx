  ...
  useEffect(() => {
    if (dashboardStats) {
      setMetrics({
        ...
        totalReturns: 0,
        avgTransactionValue:
          dashboardStats.totalDailySales > 0
            ? Math.round(
                dashboardStats.revenueToday / dashboardStats.totalDailySales,
              )
            : 0,
        pendingPayments: 0,
      });
          ...
          <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            {metricCards.map((m, i) => (
              <button
                ...
                className="admin-metric-card group text-left transition-all hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <Card className="lg:col-span-2 rounded-[2.5rem] border-slate-200 shadow-sm overflow-hidden">
              <CardHeader className="bg-slate-50/50 border-b border-slate-100 p-8">
                ...
                <CardDescription>
                  Live tracking for your current shift
                </CardDescription>
              </CardHeader>
                  ...
                  <div className="p-6 rounded-3xl bg-slate-50 border border-slate-100 flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-black text-slate-900">
                        ETB {metrics.todaySales.toLocaleString()}
                      </p>
                    </div>
                  </div>
                    ...
                    {[
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div
                            className={`w-10 h-10 rounded-xl ${task.bg} ${task.color} flex items-center justify-center`}
                          >
