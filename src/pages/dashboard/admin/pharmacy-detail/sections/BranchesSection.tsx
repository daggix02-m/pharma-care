...
function BranchCard({ branch }: BranchCardProps) {
  ...
  const config = statusConfig[branch.status as keyof typeof statusConfig] || statusConfig.active;
      ...
      <CardContent className='space-y-3'>
        ...
        {branch.licenseDetails && (
          <div className='p-3 bg-muted/30 rounded-lg space-y-2'>
            <div className='flex justify-between items-center'>
              ...
              <span className='text-xs font-medium'>{branch.licenseDetails.licenseNumber}</span>
            </div>
            <div className='flex justify-between items-center'>
              <span className='text-xs text-muted-foreground'>Issuing Authority</span>
              <span className='text-xs font-medium'>{branch.licenseDetails.issuingAuthority}</span>
            </div>
            <div className='flex justify-between items-center'>
              ...
              >
                {new Date(branch.licenseDetails.expiryDate).toLocaleDateString()}
              </span>
            </div>
          </div>
        )}
        ...
        {branch.operatingHours && (
          <div className='p-3 bg-muted/30 rounded-lg'>
            <p className='text-xs font-medium text-muted-foreground mb-2'>Operating Hours</p>
            <div className='grid grid-cols-7 gap-1'>
              {Object.entries(branch.operatingHours as Record<string, string>).map(
                ([day, hours]) => (
                  <div key={day} className='text-center'>
                    <p className='text-[10px] uppercase text-muted-foreground'>{day.slice(0, 3)}</p>
