...
export const useRealtimeSalesData = () => {
  ...
  const managerSalesData = useQuery(
    (api.manager as any).getReportsSales,
    "daily",
  );
  ...
  const data: any = {
    totalRevenue:
      (salesData as any)?.totalSales || (salesData as any)?.total_revenue || 0,
    salesCount:
      (salesData as any)?.salesCount ||
      (salesData as any)?.transactionCount ||
      0,
